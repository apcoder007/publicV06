...
angular.module("materialCalendar").constant("materialCalendar.config",{version:"0.2.13",debug:document.domain.indexOf("localhost")>-1});
angular.module("materialCalendar").config(["materialCalendar.config","$logProvider","$compileProvider",function(config,$logProvider,$compileProvider){
  if(config.debug){$logProvider.debugEnabled(false);
    ...
    angular.module("materialCalendar").directive('compile',['$compile',function($compile){
      return function(scope,element,attrs){
        ...
        angular.module("materialCalendar").service("materialCalendar.Calendar",[function(){function Calendar(year,month,options){var now=new Date();this.setWeekStartsOn=function(i){var d=parseInt(i||0,10);if(!isNaN(d)&&d>=0&&d<=6){this.weekStartsOn=d;}else{this.weekStartsOn=0;}
return this.weekStartsOn;};
this.setStartDateOfMonth=function(i){var d=parseInt(i||1,10);if(!isNaN(d)&&d>=1&&d<=31){this.startDateOfMonth=d;}else{this.startDateOfMonth=1;}
...
this.init=function(year,month){
  var now=new Date();
  this.year=angular.isDefined(year)?year:now.getFullYear();this.month=angular.isDefined(month)?month:now.getMonth();
  ...
  var monthLength=daysInMonth[this.month];
    if(this.month===1){
        ...
        if(angular.isDefined(this.options.startDateOfMonth)){
            ...
            var date=angular.copy(this.start);if(date.getDate()===1){
              while(date.getDay()!==this.weekStartsOn){
                date.setDate(date.getDate()- 1);monthLength++;
              }
            }
            if(this.noOfDays!==0){
  ...
  var injectCss=function(){
    ...
    if(!document.getElementById(styleId)){
      ...
      $scope.sameMonth=function(date){
        var d=angular.copy(date);return d.getFullYear()===$scope.calendar.year&&d.getMonth()===$scope.calendar.month;
      };
...
if($scope.disableFutureSelection&&date>new Date()){
    ...
    $scope.$watch("weekLayout",function(){
      year=$scope.calendar.year;month=$scope.calendar.month;bootstrap();
      });
      var handleCb=function(cb,data){(cb||angular.noop)(data);
      };
      var dateFind=function(arr,date){
        var index=-1;
        angular.forEach(arr,function(d,k){
          if(index<0){
            if(angular.equals(date,d)){index=k;
            }}});
            return index;
          };
          $scope.isActive=function(date){
            ...
            $scope.showDayClick = function(date){
            ...
            $scope.handleDayClick=function(date){
              ...
              var active=angular.copy($scope.active);
                ...
                var init=function(){
                  $scope.calendar=new Calendar(year,month,{weekStartsOn:$scope.weekStartsOn||0,startDateOfMonth:$scope.startDateOfMonth||1,noOfDays:$scope.noOfDays||0});if($scope.templateUrl){return $templateRequest($scope.templateUrl);}
                ...
                var getDayContent=function(date){
                  ...
                  if(angular.isObject(result)&&"function"===typeof result.success){
                    ...
                    var setData=function(){
                      ...
                      window.data=$scope.data;var bootstrap=function(){
                        init().then(function(contents){setTemplate(contents);setData();});};
